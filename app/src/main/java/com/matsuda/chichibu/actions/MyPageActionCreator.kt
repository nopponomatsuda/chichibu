...
object MyPageActionCreator {
    ...
    private fun query(appSyncClient: AWSAppSyncClient) {
        appSyncClient.query(GetChichibuQuery.builder().build())
            .responseFetcher(AppSyncResponseFetchers.CACHE_AND_NETWORK)
            .enqueue(callback)
    }

    private val callback = object : GraphQLCall.Callback<GetChichibuQuery.Data>() {
        override fun onResponse(response: com.apollographql.apollo.api.Response<GetChichibuQuery.Data>) {
            Log.i("Results", response.data()?.chichibu.toString())
        }
